...
async function loadSpreadsheet(contentUrl: string, filename: string): Promise<Sheet[]> {
...
const spreadsheets = createAsyncLruCache<string, Sheet[]>({ maxEntries: 8 });

export function preloadSpreadsheet(
  contentUrl: string,
  filename: string,
): Promise<Sheet[]> {
  return spreadsheets.get(contentUrl, () => loadSpreadsheet(contentUrl, filename));
}
...
export function SpreadsheetPreview({
  ...
  const cached = spreadsheets.peek(contentUrl);
  const [sheets, setSheets] = useState<Sheet[]>(() => cached ?? []);
  ...
  useEffect(() => {
    let cancelled = false;
    const immediate = spreadsheets.peek(contentUrl);
    setSheets(immediate ?? []);
    setActiveSheet(0);
    setSelected(null);
    setError(null);
    void preloadSpreadsheet(contentUrl, filename)
      .then((next) => {
        if (!cancelled) {
          setSheets(next);
        }
