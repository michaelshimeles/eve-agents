  ...
  return (
    <div className="flex min-h-0 flex-1">
          ...
          <div role="tablist" aria-label="Artifact scope" className="flex min-w-0 flex-1 gap-1">
            {(["thread", "all"] as const).map((value) => (
              <button
                ...
                className={cn(
                  "min-w-0 flex-1 rounded-md px-2 py-1 text-xs capitalize",
                  scope === value
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        <div className="border-b border-kumo-hairline bg-kumo-recessed px-3 py-1.5">
          <p className="text-[11px] text-kumo-subtle">
            Workspace access is deployment-wide until web authentication is configured.
          </p>
        </div>
        {detailLoading ? (
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-2 p-6 text-center">
            ...
            <p className="max-w-xs text-xs text-kumo-subtle">
              Upload a document, or ask Ruth to create one. Revisions, drafts, comments, and
              shares stay attached to the artifact.
            </p>
            <Button size="sm" variant="secondary" onClick={() => uploadInput.current?.click()}>
              <PlusIcon />
              Upload artifact
            </Button>
          </div>
