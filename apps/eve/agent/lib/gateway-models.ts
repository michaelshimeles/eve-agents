...
export interface GatewayModelOption {
  ...
  /** Provider slug from the Gateway catalog, e.g. `anthropic`. */
...
function modalitiesOf(value: unknown): {
  inputModalities: string[];
  outputModalities: string[];
} {
  ...
  const record = value as Record<string, unknown>;
  return {
    inputModalities: stringArray(record.input),
    outputModalities: stringArray(record.output),
  };
}
...
function mapRestModel(raw: GatewayRestModel): GatewayModelOption | null {
  ...
  const modalities = modalitiesOf(raw.modalities);
  return {
    id,
    name,
    ...
    provider: asString(raw.owned_by),
    ...modalities,
