...
async function executeGatewayComputerAction(
  ...
  switch (action.action) {
    case "screenshot":
      ...
      break;
    case "click": {
      const x = requiredActionNumber(action, "x");
      const y = requiredActionNumber(action, "y");
    ...
    case "drag": {
      const startX = requiredActionNumber(action, "start_x");
      const startY = requiredActionNumber(action, "start_y");
      const endX = requiredActionNumber(action, "end_x");
      const endY = requiredActionNumber(action, "end_y");
    ...
    case "type":
      await api(`/computers/${computer.id}/type`, {
        method: "POST",
        body: { text: requiredActionString(action, "text") },
        signal,
      });
      message = "Typed the requested text.";
      break;
    case "key": {
      const key = requiredActionString(action, "key");
    ...
    case "bash": {
      const result = await api<{ output?: string; exit_code?: number }>(
        ...
        {
          method: "POST",
          body: { command: requiredActionString(action, "command") },
          signal,
        },
      );
...
async function gatewayComputerTask(input: {
  ...
  const result = await runComputerUseLoop({
    ...
    initialScreenshot: (signal) => computerScreenshotBase64(input.computer, signal),
    executeAction: (action, signal) =>
      executeGatewayComputerAction(input.computer, action, signal),
    timeoutSeconds,
    timeoutMessage: "The AI Gateway computer task reached its time limit.",
