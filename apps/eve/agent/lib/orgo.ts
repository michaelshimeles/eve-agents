...
export const orgo = {
  ...
  /**
   * The desktop plus, when it is up, everything a VNC client needs to watch
   * the screen. A frozen VM has no instance behind it yet — Orgo blanks the
   * connection fields — so `connection` is null until it is woken.
   */
  async live(
    signal?: AbortSignal,
   ...
   * absolute URL or inline base64. Normalize all three to something the owner
   * can open in chat.
   */
  async screenshot(
    signal?: AbortSignal,
  ): Promise<{ computer: Computer; imageUrl: string | null; inlineBytes: number | null }> {
    ...
    if (/^https?:\/\//.test(image)) return { computer, imageUrl: image, inlineBytes: null };
    if (image.startsWith("/")) {
      return { computer, imageUrl: `${siteBase()}${image}`, inlineBytes: null };
    }
    if (image.length === 0) return { computer, imageUrl: null, inlineBytes: null };

    const base64 = image.startsWith("data:") ? (image.split(",", 2)[1] ?? "") : image;
    return {
      computer,
      imageUrl: null,
      inlineBytes: Math.floor((base64.length * 3) / 4),
    };
  },
