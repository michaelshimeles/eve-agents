...
export const DELIVERY_TARGETS = ["origin", "web", "telegram", "imessage"] as const;
...
export async function deliveryView(): Promise<DeliveryView> {
  const [target, chatId, pairing] = await Promise.all([
    getDeliveryTarget(),
    ownerTelegramChatId(),
    runTool(verifiedIMessagePairing()).catch(() => null),
  ]);
  return { target, telegramLinked: chatId !== null, imessagePaired: pairing !== null };
}
