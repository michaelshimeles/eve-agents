...
// the UI instead of the deployment's environment (today: the Orgo API key and
...
export const settingsStore = {
  ...
  /**
   * Reads through to the database, skipping the stale-while-revalidate path.
   * For values acted on the moment they are read (delivery routing), where a
   * cached read could apply a preference the owner already changed — writes
   * only invalidate this process's cache, and route handlers and the agent
   * run as separate instances.
   */
  async getFresh(name: string): Promise<string | null> {
    if (!hasDatabase()) return null;
    try {
