...
import {
  ...
  type OpaqueAttachmentAccess,
  verifyAttachmentAccess,
  verifyOpaqueAttachmentAccess,
} from "../imessage-signature";
import { DatabaseError, Db, describeDatabaseError } from "./db";
import {
  fetchPublicMedia,
  UnsafeDeploymentUrlError,
  validateDeploymentUrl,
} from "./imessage/security";
import { IMessageRichStore as IMessageRichStoreService } from "./imessage/store";
import type {
  IMessageCommandResult,
  IMessageOperation,
  IMessageTarget,
} from "./imessage/schema";
...
//   IMessagePairing — deployment side: the pairing row, inbound claims,
//     owner-activated group spaces, and HTTP calls to the router.
//   IMessageRouter — router side: the handle registry, the group-space
//     registry, OTP pair requests, and the actual Spectrum sends (SDK, or the
//     stub behind SPECTRUM_API_BASE_URL).
//
// Group chats: a group space carries messages from people who are not paired
// with any deployment. The trust rule is owner opt-in per space — a space
// only becomes live after the paired owner speaks in it. The router records
// that as a space -> handle binding and forwards other participants' messages
// in bound spaces to that deployment; the deployment keeps its own activation
// record and re-checks it, so accepting a guest never depends on the router
// alone. Replies address the space id, which is also the only send target the
// router accepts beyond the deployment's own paired handle.
...
export class IMessageError extends Data.TaggedError("IMessageError")<{
  ...
  /** Provider classification; only documented transient failures may replay. */
...
interface ContentArm {
  ...
  readonly isAudioMessage?: unknown;
  readonly url?: unknown;
  readonly items?: unknown;
}

/** Provider media identifiers are opaque, but empty/whitespace values are unusable. */
export function isUsableIMessageProviderId(value: unknown): value is string {
...
export interface InboundFileRef {
  ...
  readonly isVoiceMemo?: boolean;
}

export interface InboundContent {
  readonly text: string | null;
  readonly files: readonly InboundFileRef[];
  readonly audio: readonly InboundFileRef[];
  readonly processable: readonly InboundFileRef[];
}
...
export function renderInboundContent(content: unknown): InboundContent {
  ...
  switch (arm.type) {
    ...
    case "attachment": {
          ...
          audio: [
            {
              ...
              name,
              mimeType: mime,
              ...(typeof arm.isAudioMessage === "boolean"
                ? { isVoiceMemo: arm.isAudioMessage }
                : {}),
            },
          ],
          processable: [],
      ...
      if (
        ...
        return {
          ...
          files: [],
          audio: [],
          processable: [{ id, name, mimeType: mime }],
        };
      }
      ...
      return { text: note, files: [], audio: [], processable: [] };
    }
    case "voice": {
      // Spectrum 12.x emits native iMessage voice memos as their own content
      // arm. Webhook serialization removes read()/stream(), but preserves the
      // provider attachment id used by the router's capability URL.
      const name = typeof arm.name === "string" ? arm.name : "voice memo.m4a";
      const mime = typeof arm.mimeType === "string" ? arm.mimeType : "audio/mp4";
      const id = isUsableIMessageProviderId(arm.id) ? arm.id : "";
      const size = typeof arm.size === "number" ? arm.size : 0;
      if (id.length > 0 && mime.startsWith("audio/") && size <= 100 * 1024 * 1024) {
        return {
          text: null,
          files: [],
          audio: [{ id, name, mimeType: mime, isVoiceMemo: true }],
          processable: [],
        };
      }
      // Some Photon webhook versions omit the provider attachment id. Do not
      // silently acknowledge those messages as empty: wake Ruth so she can ask
      // the sender to retry or type the request.
      return {
        text:
          "[The sender sent a native voice memo, but its audio was unavailable. Ask them to retry or type the request.]",
        files: [],
        audio: [],
        processable: [],
      };
    ...
    case "richlink":
      return typeof arm.url === "string"
        ? { text: arm.url, files: [], audio: [], processable: [] }
        : none;
    case "group": {
      ...
      const audio = parts.flatMap((part) => part.audio);
      const processable = parts.flatMap((part) => part.processable);
      if (
        texts.length === 0 &&
        files.length === 0 &&
        audio.length === 0 &&
...
const PairingRow = Schema.Struct({
  ...
  deployment_url: Schema.optionalKey(Schema.NullOr(Schema.String)),
...
const TranscriptAttachmentRow = Schema.Struct({
  name: Schema.String,
...
const TranscriptRow = Schema.Struct({
  ...
  chat_type: Schema.Literals(["dm", "group"]),
  space_id: Schema.NullOr(Schema.String),
  phone: Schema.NullOr(Schema.String),
  message_id: Schema.NullOr(Schema.String),
  text: Schema.NullOr(Schema.String),
  attachments: Schema.Array(TranscriptAttachmentRow),
...
type TranscriptRow = typeof TranscriptRow.Type;

export interface IMessageTranscriptAttachment {
  readonly name: string;
  readonly mimeType: string;
}

export interface IMessageTranscriptEntry {
  readonly id: string;
  ...
  readonly text: string | null;
  readonly attachments: readonly IMessageTranscriptAttachment[];
...
function validatePairingUrl(raw: string): Effect.Effect<string, IMessageError> {
  return Effect.tryPromise({
    try: () => validateDeploymentUrl(raw),
    catch: (cause) =>
      new IMessageError({
        reason: "validation",
        detail:
          cause instanceof UnsafeDeploymentUrlError
            ? cause.message
...
interface PostJsonOptions {
  ...
  readonly acceptCommandFailure?: boolean;
}
...
/**
 * One POST against the router with no implicit retry. OTP pairing and legacy
 * sends must remain single-attempt. The v2 command service may call this
 * again only with the same durable commandId, which makes a lost
...
function postJson(options: PostJsonOptions): Effect.Effect<unknown, IMessageError> {
  return Effect.tryPromise({
    try: async () => {
      ...
      if (!response.ok) {
        if (options.acceptCommandFailure) {
          try {
            const parsed = JSON.parse(text) as { ok?: unknown };
            if (typeof parsed.ok === "boolean") return parsed;
          } catch {
            // Fall through to the normal router error.
          }
        }
...
/**
 * Where an outbound text goes: a handle opens (or reuses) the 1:1
 * conversation with that person; a space id addresses an existing
 * conversation — the only way to reply into a group chat.
 */
type SendTarget = { readonly handle: string } | { readonly space: string };
...
// Text sends take a SendTarget so replies can address a group space; the
// other operations stay DM-scoped (handle-addressed) for now.
interface SpectrumOps {
  readonly send: (target: SendTarget, text: string, phone: string | null) => Promise<void>;
  readonly effectText: (
    handle: string,
    text: string,
  ...
  readonly attachment: (
    handle: string,
    file: AttachmentFile & { readonly bytes: Buffer; readonly resolvedContentType: string },
    phone: string | null,
  ) => Promise<void>;
  ...
  readonly background: (
    ...
    input:
      | "clear"
      | { readonly bytes: Buffer; readonly contentType: string },
    phone: string | null,
  ) => Promise<void>;
...
function spectrumSdkOps(): Promise<SpectrumOps> {
  sdkOps ??= (async () => {
    ...
    // A handle resolves to (or creates) the 1:1 conversation; a space id
    // addresses an existing conversation, which is how group chats are
    // reached — `im.space.get` never creates one.
    const spaceFor = async (target: SendTarget, phone: string | null) => {
      if ("space" in target) {
        return phone === null ? im.space.get(target.space) : im.space.get(target.space, { phone });
      }
    ...
    return {
      send: async (target, text, phone) => {
        const space = await spaceFor(target, phone);
      ...
      attachment: async (handle, file, phone) => {
        ...
        await space.send(
          attachment(file.bytes, {
            ...
            mimeType: file.resolvedContentType,
          }),
        );
      },
      ...
      background: async (handle, input, phone) => {
        ...
        await space.send(
          input === "clear"
            ? background("clear")
            : background(input.bytes, { mimeType: input.contentType }),
        );
      },
 ...
 */
function sendText(target: SendTarget, text: string, phone?: string): Effect.Effect<void, IMessageError> {
  return Effect.tryPromise({
    try: async () => {
      ...
      const send =
        stub !== null
          ? (t: SendTarget, chunk: string, p: string | null) =>
              postToStub(stub, "/v1/send", {
                handle: "handle" in t ? t.handle : null,
                spaceId: "space" in t ? t.space : null,
                text: chunk,
                phone: p,
              })
          : (await spectrumSdkOps()).send;
      for (const chunk of splitMessageText(text)) {
        await send(target, chunk, line);
      }
    },
 ...
 * accepted — no plain http, no localhost, no literal IP hosts. The actual
 * download is separately DNS-resolved, public-range checked, and pinned.
 */
...
function sendAttachmentContent(
  ...
  return Effect.tryPromise({
    try: async () => {
      ...
      const downloaded = await fetchPublicMedia(file.url);
      await (await spectrumSdkOps()).attachment(
        handle,
        {
          ...file,
          bytes: Buffer.from(downloaded.data),
          resolvedContentType:
            file.contentType ?? downloaded.contentType ?? "application/octet-stream",
        },
        line,
      );
    },
  ...
  return Effect.tryPromise({
    try: async () => {
      ...
      const resolved =
        input === "clear"
          ? "clear"
          : await fetchPublicMedia(input).then((image) => ({
              bytes: Buffer.from(image.data),
              contentType: image.contentType ?? "image/png",
            }));
      await (await spectrumSdkOps()).background(handle, resolved, line);
    },
    catch: asSpectrumError,
  });
}
...
export class IMessagePairing extends Context.Service<IMessagePairing, {
  ...
  readonly claimInbound: (input: {
    ...
    /** Paired owner whose DM/group conversation this delivery belongs to. */
    readonly ownerHandle: string;
    /** Actual sender; group guests remain separately attributed and batched. */
    readonly handle: string;
    readonly text: string | null;
    readonly attachments: readonly IMessageTranscriptAttachment[];
    readonly chatType: "dm" | "group";
    ...
    /** Provider delivery time, or route-entry time when the provider omitted it. */
    readonly receivedAtMs: number;
    /** High-resolution route-entry order used when provider timestamps tie. */
   ...
   * the request holding the newest undispatched claim for the sender within
   * the space drains the batch (every undispatched text, oldest first);
   * everyone else defers to it. Scoped to (handle, space) so one sender's
   * DM and group-chat bursts never fold into one dispatch. Stale in-flight
   * rows (crashed dispatchers) are reclaimed.
   */
  readonly settleInbound: (input: {
    ...
    IMessageStoreError
  >;
  readonly reconcileInbound: (input: {
    readonly messageId: string;
    readonly kind: "edited" | "unsent";
    readonly text?: string;
  }) => Effect.Effect<"pending_updated" | "already_started" | "missing", IMessageStoreError>;
  /**
   * An owner-authored /new supersedes every earlier delivery in this
   * conversation. Mark both queued and already-dispatching rows terminal so a
   * sleeping debounce request cannot enter the freshly reset session, and a
   * late model event can be refused before it reaches Spectrum.
   */
  readonly discardInboundForReset: (input: {
    readonly ownerHandle: string;
    readonly spaceId: string;
    readonly resetMessageId: string;
    /** Ordering cutoff carried by the owner-authored /new delivery. */
    readonly resetAtMs: number;
    readonly resetRouteReceivedAtMs: number;
  }) => Effect.Effect<void, IMessageStoreError>;
  /** True unless /new terminally superseded or failure cleanup removed a row. */
  readonly isInboundBatchCurrent: (
    messageIds: readonly string[],
  ) => Effect.Effect<boolean, IMessageStoreError>;
  /**
   * One short durable lease serializes a conversation's Spectrum delivery
   * boundary with /new. A crashed holder expires; release still matches the
   * random token so an old request cannot unlock its successor.
   */
  readonly tryAcquireDeliveryLock: (input: {
    readonly ownerHandle: string;
    readonly spaceId: string;
  }) => Effect.Effect<string | null, IMessageStoreError>;
  readonly releaseDeliveryLock: (input: {
    readonly ownerHandle: string;
    readonly spaceId: string;
  ...
  /** Latest provider-facing messages and their delivery state, newest first. */
  readonly listTranscript: (
    limit: number,
  ) => Effect.Effect<readonly IMessageTranscriptEntry[], IMessageStoreError>;
  /**
   * Mark a group space as owner-activated: the paired handle spoke in it, so
   * other participants' messages in that space may reach the agent. This is
   * the deployment's own record — it must not depend on the router's routing
   * decisions, the same posture as the sender check on inbound.
   */
  readonly activateSpace: (input: {
    readonly spaceId: string;
    readonly handle: string;
  }) => Effect.Effect<void, IMessageStoreError>;
  /** True when the space was activated by the given (current) paired handle. */
  readonly isSpaceActive: (input: {
    readonly spaceId: string;
    readonly handle: string;
  }) => Effect.Effect<boolean, IMessageStoreError>;
  readonly sendCommand: (input: {
    ...
    readonly phone: string;
    readonly target: IMessageTarget;
    readonly operation: IMessageOperation;
    readonly payload: unknown;
  }) => Effect.Effect<IMessageCommandResult, IMessageStoreError>;
  readonly sendReply: (input: {
    readonly handle: string;
    readonly text: string;
    /** Line the conversation lives on, so the reply comes from the same number. */
    readonly phone?: string;
    /** Group space to reply into; without it the reply is a DM to `handle`. */
...
export const IMessagePairingLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const decodeTranscriptRows = Schema.decodeUnknownEffect(Schema.Array(TranscriptRow));
    ...
    const ensureTables = Effect.suspend(() =>
      tablesReady
        ...
        : Effect.gen(function* () {
            ...
            yield* database.query(
              `ALTER TABLE imessage_pairing ADD COLUMN IF NOT EXISTS deployment_url text`,
            );
            yield* database.query(
              `CREATE TABLE IF NOT EXISTS imessage_inbound (
                 ...
                 route_received_at double precision,
            ...
            // Delivery ordering is separate from database arrival: a webhook
            // that began before /new may not reach this insert until later.
            yield* database.query(
              `ALTER TABLE imessage_inbound ADD COLUMN IF NOT EXISTS received_at timestamptz`,
            );
            yield* database.query(
              `UPDATE imessage_inbound SET received_at = claimed_at WHERE received_at IS NULL`,
            );
            yield* database.query(
              `ALTER TABLE imessage_inbound ALTER COLUMN received_at SET DEFAULT now()`,
            );
            yield* database.query(
              `ALTER TABLE imessage_inbound ALTER COLUMN received_at SET NOT NULL`,
            );
            yield* database.query(
              `ALTER TABLE imessage_inbound ADD COLUMN IF NOT EXISTS route_received_at double precision`,
            );
            yield* database.query(
              `UPDATE imessage_inbound
                  SET route_received_at = EXTRACT(EPOCH FROM claimed_at) * 1000.0
                WHERE route_received_at IS NULL`,
            );
            yield* database.query(
              `ALTER TABLE imessage_inbound
                 ALTER COLUMN route_received_at
                 SET DEFAULT (EXTRACT(EPOCH FROM clock_timestamp()) * 1000.0)`,
            );
            yield* database.query(
              `ALTER TABLE imessage_inbound ALTER COLUMN route_received_at SET NOT NULL`,
            );
            yield* database.query(
              `CREATE TABLE IF NOT EXISTS imessage_reset_barrier (
                 handle text NOT NULL,
                 space_id text NOT NULL,
                 reset_at timestamptz NOT NULL,
                 reset_route_received_at double precision,
                 PRIMARY KEY (handle, space_id)
               )`,
            );
            yield* database.query(
              `ALTER TABLE imessage_reset_barrier
                 ADD COLUMN IF NOT EXISTS reset_route_received_at double precision`,
            );
            yield* database.query(
              `UPDATE imessage_reset_barrier
                  SET reset_route_received_at = EXTRACT(EPOCH FROM reset_at) * 1000.0
                WHERE reset_route_received_at IS NULL`,
            );
            yield* database.query(
              `ALTER TABLE imessage_reset_barrier
                 ALTER COLUMN reset_route_received_at
                 SET DEFAULT (EXTRACT(EPOCH FROM clock_timestamp()) * 1000.0)`,
            );
            yield* database.query(
              `ALTER TABLE imessage_reset_barrier
                 ALTER COLUMN reset_route_received_at SET NOT NULL`,
            );
            yield* database.query(
              `CREATE TABLE IF NOT EXISTS imessage_delivery_lock (
                 handle text NOT NULL,
                 space_id text NOT NULL,
                 token text NOT NULL,
            ...
            yield* database.query(
              `CREATE TABLE IF NOT EXISTS imessage_space (
                 space_id text PRIMARY KEY,
                 handle text NOT NULL,
                 activated_at timestamptz NOT NULL DEFAULT now(),
                 last_owner_at timestamptz NOT NULL DEFAULT now()
               )`,
            );
            yield* database.query(
              `CREATE TABLE IF NOT EXISTS imessage_transcript (
            ...
            yield* database.query(
              `CREATE INDEX IF NOT EXISTS imessage_transcript_created_at_idx
                 ON imessage_transcript (created_at DESC)`,
            );
            tablesReady = true;
          }),
    );
    ...
    const readRow = Effect.gen(function* () {
      ...
      const rows = yield* database.query(
        `SELECT router_url, deployment_url, handle, status, pairing_id, secret,
    ...
    const verified = Effect.gen(function* () {
      ...
      return {
        routerUrl: row.router_url,
        ...(row.deployment_url !== null && row.deployment_url !== undefined
    ...
    const transcriptView = (row: TranscriptRow): IMessageTranscriptEntry => ({
      id: row.entry_id,
    ...
    const describeTranscriptFailure = (failure: unknown): string => {
    ...
    const bestEffortTranscript = <A, E>(
      operation: string,
      ...
      effect.pipe(
        Effect.tapError((failure) =>
          Effect.sync(() => {
            console.warn(
              `iMessage transcript ${operation} failed.`,
              describeTranscriptFailure(failure),
            );
          }),
        ),
        Effect.ignore,
    ...
    const outbound = (
      ...
      send: Effect.Effect<void, IMessageError>,
    ): Effect.Effect<void, IMessageError> =>
      Effect.suspend(() => {
        const entryId = `outbound:${randomUUID()}`;
        const persist = (
          ...
          bestEffortTranscript(
            ...
            Effect.gen(function* () {
              yield* ensureTables;
              yield* database.query(
                `INSERT INTO imessage_transcript (
                   entry_id, direction, kind, handle, role, chat_type, space_id,
                   phone, message_id, text, attachments, status, error
                 )
                 VALUES ($1, 'outbound', $2, $3, 'assistant', $4, $5, $6, NULL, $7, $8::jsonb, $9, $10)
                ...
                [
                  ...
                  input.kind,
                  input.handle,
                  input.space === undefined ? "dm" : "group",
                  input.space ?? null,
                  input.phone ?? null,
                  input.text,
        ...
        return Effect.gen(function* () {
          yield* persist("sending");
          yield* send.pipe(
            Effect.tap(() => persist("sent")),
            Effect.tapError((failure) =>
              persist("failed", describeTranscriptFailure(failure).slice(0, 1000)),
            ),
          );
        });
      });
    ...
    return {
      ...
      begin: (input) =>
        Effect.gen(function* () {
          ...
          const routerUrl = yield* validatePairingUrl(input.routerUrl);
          const deploymentUrl = yield* validatePairingUrl(input.deploymentUrl);
          ...
          yield* ensureTables;
          yield* database.query(
            `INSERT INTO imessage_pairing
               (id, router_url, deployment_url, handle, status, pairing_id, secret, requested_at, verified_at)
             VALUES (1, $1, $2, $3, 'pending', $4, NULL, now(), NULL)
             ON CONFLICT (id) DO UPDATE
               SET router_url = EXCLUDED.router_url,
                   ...
                   requested_at = now(),
                   verified_at = NULL`,
            [routerUrl, deploymentUrl, handle, pairingId],
          );
          return view(yield* readRow);
        }),
      ...
      unpair: () =>
        Effect.gen(function* () {
          ...
          if (row !== null && row.status === "verified" && row.secret !== null) {
            // The router must confirm the removal before local state goes
            // away: its registration keeps group-space bindings routing here,
            // and once this deployment forgets the secret those bindings
            // would strand every bound group on a dead route that no other
            // paired participant can claim. The only answer that reads as
            // already-gone is a 401 — the router's own authorization saying
            // this handle is not paired with this secret (removed, or
            // re-paired elsewhere with a rotated secret). Anything else,
            // including a 404 (the endpoint can 404 while the router is
            // temporarily deployed without its SPECTRUM_* config, with the
            // registration rows very much alive underneath), keeps the
            // pairing so the owner can retry.
            yield* postJson({
              url: `${row.router_url}/api/imessage/unpair`,
              json: { handle: row.handle },
              bearer: row.secret,
            }).pipe(
              Effect.catch((error: IMessageError) =>
                error.status === 401
                  ? Effect.void
                  : Effect.fail(
                      new IMessageError({
                        reason: "router",
                        detail: `the router did not confirm unpairing (${error.detail ?? "unreachable"}) — the pairing was kept so group chats stay routable; try again`,
          ...
          // Group activations belong to the pairing that made them.
          yield* database.query(`DELETE FROM imessage_space`);
          return view(null);
        }),
      ...
      claimInbound: (input) =>
        Effect.gen(function* () {
          yield* requireDatabase;
          yield* ensureTables;
          // Claim and reset take the same conversation advisory lock. This
          // closes the concurrent-insert hole: whichever transaction wins is
          // ordered before the other, so reset either sweeps this row or this
          // claim sees the persisted cutoff and refuses itself.
          const transaction = yield* database.transaction([
            {
              sql: `SELECT pg_advisory_xact_lock(hashtext($1), hashtext($2))`,
              params: [input.ownerHandle, input.spaceId],
            },
            {
              sql: `INSERT INTO imessage_inbound
                      (message_id, space_id, handle, text, received_at, route_received_at)
                    SELECT $1, $2, $3, $4, to_timestamp($5 / 1000.0), $7
                     WHERE NOT EXISTS (
                       SELECT 1 FROM imessage_reset_barrier
                        WHERE handle = $6
                          AND space_id = $2
                          AND (
                            reset_at > to_timestamp($5 / 1000.0)
                            OR (
                              reset_at = to_timestamp($5 / 1000.0)
                              AND reset_route_received_at >= $7
                            )
                          )
                     )
          ...
          if (inserted.length === 0) {
            ...
            status = (existing[0] as { status?: string } | undefined)?.status ?? "ok";
          }
          ...
          if (status !== "claimed" && status !== "dispatching") return "done" as const;

          // Provider retries refresh the deterministic row instead of adding
          // duplicate transcript entries. A transient logging failure can
          // therefore repair itself on the next delivery attempt.
          yield* bestEffortTranscript(
            "inbound receive",
            database.query(
              `INSERT INTO imessage_transcript (
                 entry_id, direction, kind, handle, role, chat_type, space_id,
                 phone, message_id, text, attachments, status, error, provider_at
               )
               VALUES (
                 $1, 'inbound', $2, $3, $4, $5, $6, $7, $8, $9, $10::jsonb,
                 'received', NULL, to_timestamp($11 / 1000.0)
               )
               ON CONFLICT (entry_id) DO UPDATE
                 SET status = 'received',
                     error = NULL,
          ...
          return inserted.length > 0 ? ("new" as const) : ("pending" as const);
        }),
        ...
        Effect.gen(function* () {
          ...
          const newest = yield* database.query(
            `SELECT message_id FROM imessage_inbound
              WHERE handle = $1 AND space_id = $2 AND ${drainable}
              ORDER BY claimed_at DESC, message_id DESC
              LIMIT 1`,
            [input.handle, input.spaceId],
          );
      ...
      reconcileInbound: (input) =>
        Effect.gen(function* () {
          ...
          if (rows.length > 0) return "pending_updated" as const;
          const existing = yield* database.query(
            `SELECT message_id FROM imessage_inbound WHERE message_id = $1`,
            [input.messageId],
          );
          return existing.length > 0 ? ("already_started" as const) : ("missing" as const);
        }),

      discardInboundForReset: (input) =>
        Effect.gen(function* () {
          yield* requireDatabase;
          yield* ensureTables;
          yield* database.transaction([
            ...
            {
              sql: `INSERT INTO imessage_reset_barrier
                      (handle, space_id, reset_at, reset_route_received_at)
                    VALUES ($1, $2, to_timestamp($3 / 1000.0), $4)
                    ON CONFLICT (handle, space_id) DO UPDATE
                      SET reset_at = CASE
                            WHEN (
                              EXCLUDED.reset_at,
                              EXCLUDED.reset_route_received_at
                            ) > (
                              imessage_reset_barrier.reset_at,
                              imessage_reset_barrier.reset_route_received_at
                            )
                            THEN EXCLUDED.reset_at
                            ELSE imessage_reset_barrier.reset_at
                          END,
                          reset_route_received_at = CASE
                            WHEN (
                              EXCLUDED.reset_at,
                              EXCLUDED.reset_route_received_at
                            ) > (
                            ...
                            THEN EXCLUDED.reset_route_received_at
                            ELSE imessage_reset_barrier.reset_route_received_at
                          END`,
              params: [
                input.ownerHandle,
                input.spaceId,
                input.resetAtMs,
                input.resetRouteReceivedAtMs,
              ],
            },
            {
              sql: `UPDATE imessage_inbound
                       SET status = 'reset', error = NULL
                     WHERE space_id = $2
                       AND message_id <> $3
                       AND (
                         received_at < (
                           SELECT reset_at FROM imessage_reset_barrier
                            WHERE handle = $1 AND space_id = $2
                         )
                         OR (
                           received_at = (
                             SELECT reset_at FROM imessage_reset_barrier
                           ...
                           AND route_received_at <= (
                             SELECT reset_route_received_at
                               FROM imessage_reset_barrier
                              WHERE handle = $1 AND space_id = $2
                           )
                         )
                       )
                       AND status IN ('claimed', 'dispatching', 'ok', 'error')`,
              params: [input.ownerHandle, input.spaceId, input.resetMessageId],
            },
          ]);
          yield* bestEffortTranscript(
            "inbound reset",
            database.query(
              `UPDATE imessage_transcript AS transcript
                  SET status = 'reset',
                      error = NULL,
                      session_id = NULL,
                      updated_at = now()
                WHERE transcript.direction = 'inbound'
                  AND transcript.message_id IN (
                    SELECT inbound.message_id
                      FROM imessage_inbound AS inbound
                     WHERE inbound.space_id = $1
                       AND inbound.status = 'reset'
                  )`,
              [input.spaceId],
            ),
          );
      ...
      isInboundBatchCurrent: (messageIds) =>
        Effect.gen(function* () {
          yield* requireDatabase;
          yield* ensureTables;
          ...
          const rows = yield* database.query(
            `SELECT message_id FROM imessage_inbound
              WHERE message_id = ANY($1::text[]) AND status <> 'reset'`,
            [uniqueIds],
          );
          return rows.length === uniqueIds.length;
        }),

      tryAcquireDeliveryLock: (input) =>
        Effect.gen(function* () {
          yield* requireDatabase;
          yield* ensureTables;
          const token = randomUUID();
          const rows = yield* database.query(
            `INSERT INTO imessage_delivery_lock
                (handle, space_id, token, expires_at)
              VALUES ($1, $2, $3, now() + interval '45 seconds')
              ON CONFLICT (handle, space_id) DO UPDATE
                SET token = EXCLUDED.token,
                    expires_at = EXCLUDED.expires_at
                WHERE imessage_delivery_lock.expires_at < now()
              RETURNING token`,
            [input.ownerHandle, input.spaceId, token],
          );
          return rows.length > 0 ? token : null;
        }),

      releaseDeliveryLock: (input) =>
        Effect.gen(function* () {
          yield* requireDatabase;
          yield* ensureTables;
          yield* database.query(
            `DELETE FROM imessage_delivery_lock
              WHERE handle = $1 AND space_id = $2 AND token = $3`,
            [input.ownerHandle, input.spaceId, input.token],
          );
        }),
      ...
      recordInboundBatch: (messageIds, result) =>
        Effect.gen(function* () {
          ...
          yield* bestEffortTranscript(
            ...
            database.query(
              `UPDATE imessage_transcript AS transcript
                  SET status = CASE WHEN $2 = 'ok' THEN 'dispatched' ELSE 'failed' END,
                      error = $3,
                      session_id = $4,
                      updated_at = now()
                WHERE transcript.direction = 'inbound'
                  AND transcript.message_id = ANY($1::text[])
                  AND EXISTS (
                    SELECT 1
                      FROM imessage_inbound AS inbound
                     WHERE inbound.message_id = transcript.message_id
                       AND inbound.status = $2
                  )`,
              [[...messageIds], result.status, result.error ?? null, result.sessionId ?? null],
            ),
          );
        }),
            ...
            database.query(
              `UPDATE imessage_transcript AS transcript
                  SET status = CASE
                        ...
                        WHEN transcript.message_id = $2 THEN 'failed'
                        ELSE 'received'
                      END,
                      error = CASE
                        WHEN transcript.message_id = $2
                         AND NOT EXISTS (
                           SELECT 1
                             FROM imessage_inbound AS inbound
                            WHERE inbound.message_id = transcript.message_id
              ...
              [
                ...
                input.ownMessageId,
                input.error?.slice(0, 1000) ?? "Eve did not accept the message.",
              ],
            ),
          );
        }),

      listTranscript: (limit) =>
        Effect.gen(function* () {
          yield* requireDatabase;
          yield* ensureTables;
          const safeLimit = Math.max(1, Math.min(Math.trunc(limit), 200));
          const rows = yield* database.query(
            `SELECT entry_id, direction, kind, handle, role, chat_type, space_id,
                    phone, message_id, text, attachments, status, error, session_id,
                    COALESCE(provider_at, created_at)::text AS occurred_at,
                    updated_at::text AS updated_at
               FROM imessage_transcript
              ORDER BY created_at DESC
              LIMIT $1`,
            [safeLimit],
          );
          return (yield* decodeTranscriptRows(rows)).map(transcriptView);
        }),
      ...
      activateSpace: (input) =>
        Effect.gen(function* () {
          ...
          yield* ensureTables;
          yield* database.query(
            `INSERT INTO imessage_space (space_id, handle)
             VALUES ($1, $2)
             ON CONFLICT (space_id) DO UPDATE
               SET handle = EXCLUDED.handle,
                   last_owner_at = now()`,
            [input.spaceId, input.handle],
          );
        }),
      ...
      isSpaceActive: (input) =>
        Effect.gen(function* () {
          ...
          yield* ensureTables;
          const rows = yield* database.query(
            `SELECT space_id FROM imessage_space WHERE space_id = $1 AND handle = $2`,
            [input.spaceId, input.handle],
          );
      ...
      sendCommand: (input) =>
        Effect.gen(function* () {
          ...
          if (pairing.deploymentUrl === undefined) {
            return yield* Effect.fail(
              new IMessageError({
                reason: "pairing",
                detail: "pair again once to bind this deployment's v2 command identity",
              }),
            );
          }
          const command = {
              ...
              actor: {
                role: "owner",
                deploymentId: pairing.deploymentUrl,
              },
          ...
          const send = () => postJson({
              ...
              acceptCommandFailure: true,
            });
          let response = yield* send().pipe(
            Effect.catch(() => send()),
          );
          if (
            response !== null &&
          ...
          if (response === null || typeof response !== "object") {
            return yield* Effect.fail(
              new IMessageError({
                reason: "router",
                detail: "v2 command response was invalid",
              }),
            );
          }
          return response as IMessageCommandResult;
        }),
              ...
              attachments: [
                {
                  ...
                  mimeType: input.file.contentType ?? "unknown type",
                },
              ],
              ...(input.phone !== undefined ? { phone: input.phone } : {}),
            },
...
export class IMessageRouter extends Context.Service<IMessageRouter, {
  ...
  /**
   * Bind a group space to a paired handle — called when that handle speaks in
   * an unbound space. First binder wins: once a space belongs to a handle, no
   * other paired participant can take it over by speaking in the group (that
   * would reroute the conversation to their deployment and lock the original
   * owner out of replying). The binding only frees up when its handle leaves
   * the registry — unpair or supersession — after which the next paired
   * speaker may claim the space.
   */
  readonly bindSpace: (input: {
    readonly spaceId: string;
    readonly handle: string;
  }) => Effect.Effect<void, IMessageStoreError>;
  /** The deployment behind a bound group space, or null when unbound. */
  readonly lookupSpace: (
    spaceId: string,
  ) => Effect.Effect<{ handle: string; deploymentUrl: string; secret: string } | null, IMessageStoreError>;
  /** Bind an Advanced Kit DM chat GUID to its paired owner for senderless state events. */
  readonly bindChat: (input: {
    readonly phone: string;
    readonly chatId: string;
    readonly handle: string;
  }) => Effect.Effect<void, IMessageStoreError>;
  readonly lookupChat: (input: {
    readonly phone: string;
    readonly chatId: string;
  }) => Effect.Effect<{ handle: string; deploymentUrl: string; secret: string } | null, IMessageStoreError>;
  ...
  /** Resolve the current paired owner represented by a bearer secret. */
  readonly resolveActor: (
    secret: string,
  ) => Effect.Effect<{ handle: string; deploymentUrl: string } | null, IMessageStoreError>;
  ...
  readonly authorizedSend: (input: {
    ...
    readonly phone?: string;
    /** Group space to send into; it must be bound to `handle`. */
    readonly space?: string;
  }) => Effect.Effect<void, IMessageStoreError>;
  ...
  /**
   * v2 attachment capability. PII and provider identifiers stay server-side;
   * the URL contains only a random Ruth ref, expiry, conversion mode, and HMAC.
   */
  readonly signedOpaqueAttachmentFetch: (input: {
  ...
  /** Share Ruth's contact once, after the paired owner has actually written. */
  readonly shareContactAfterFirstInbound: (input: {
    readonly handle: string;
    readonly phone?: string;
  }) => Effect.Effect<void, IMessageStoreError>;
...
export const IMessageRouterLive = Layer.effect(
  ...
  Effect.gen(function* () {
        ...
        : Effect.gen(function* () {
            yield* database.query(
              `CREATE TABLE IF NOT EXISTS imessage_registry (
                 ...
                 contact_share_claimed_at timestamptz
               )`,
            );
            yield* database.query(
              `ALTER TABLE imessage_registry
                 ADD COLUMN IF NOT EXISTS contact_shared_at timestamptz,
                 ADD COLUMN IF NOT EXISTS contact_share_claimed_at timestamptz`,
            );
            yield* database.query(
            ...
            yield* database.query(
              `CREATE TABLE IF NOT EXISTS imessage_space_registry (
                 space_id text PRIMARY KEY,
                 handle text NOT NULL,
            ...
            yield* database.query(
              `CREATE TABLE IF NOT EXISTS imessage_chat_registry (
                 phone text NOT NULL,
    ...
    const decodeSpaceLookupRows = Schema.decodeUnknownEffect(
      Schema.Array(
        Schema.Struct({
          handle: Schema.String,
          deployment_url: Schema.String,
    ...
    return {
      ...
      bindSpace: (input) =>
        Effect.gen(function* () {
          yield* requireRouter;
          yield* ensureTables;
          // First binder wins: refresh our own binding, never steal a live
          // one. A binding whose handle has left the registry is dead
          // (unpair and supersession already clean these; this self-heals
          // any straggler) and may be claimed.
          yield* database.query(
            `INSERT INTO imessage_space_registry (space_id, handle)
             VALUES ($1, $2)
             ON CONFLICT (space_id) DO UPDATE
               SET handle = EXCLUDED.handle,
                   bound_at = now()
               WHERE imessage_space_registry.handle = EXCLUDED.handle
                  OR NOT EXISTS (
                       SELECT 1 FROM imessage_registry r
                        WHERE r.handle = imessage_space_registry.handle
                     )`,
            [input.spaceId, input.handle],
          );
        }),
      ...
      lookupSpace: (spaceId) =>
        Effect.gen(function* () {
          yield* requireRouter;
          yield* ensureTables;
          // The join makes an unpaired handle's bindings dead on arrival:
          // no registry row, no forward.
          const rows = yield* database.query(
            `SELECT r.handle, r.deployment_url, r.secret
               FROM imessage_space_registry s
               JOIN imessage_registry r ON r.handle = s.handle
              WHERE s.space_id = $1`,
            [spaceId],
          );
          const decoded = yield* decodeSpaceLookupRows(rows);
          if (decoded.length === 0) return null;
          return {
            handle: decoded[0].handle,
            deploymentUrl: decoded[0].deployment_url,
            secret: decoded[0].secret,
          };
        }),
      ...
      bindChat: (input) =>
        Effect.gen(function* () {
          ...
          yield* ensureTables;
          yield* database.query(
            `INSERT INTO imessage_chat_registry (phone, chat_id, handle)
             VALUES ($1, $2, $3)
             ON CONFLICT (phone, chat_id) DO UPDATE
               SET handle = EXCLUDED.handle,
                   bound_at = now()
               WHERE imessage_chat_registry.handle = EXCLUDED.handle
                  OR NOT EXISTS (
                       SELECT 1 FROM imessage_registry r
                        WHERE r.handle = imessage_chat_registry.handle
                     )`,
            [input.phone, input.chatId, input.handle],
          );
        }),
      ...
      lookupChat: (input) =>
        Effect.gen(function* () {
          ...
          const rows = yield* database.query(
            `SELECT r.handle, r.deployment_url, r.secret
               FROM imessage_chat_registry c
               JOIN imessage_registry r ON r.handle = c.handle
              WHERE c.phone = $1 AND c.chat_id = $2`,
            [input.phone, input.chatId],
          );
          const decoded = yield* decodeSpaceLookupRows(rows);
          if (decoded.length === 0) return null;
          return {
      ...
      beginPair: (input) =>
        Effect.gen(function* () {
          ...
          yield* ensureTables;
          const handle = normalizeHandle(input.handle);
          if (handle === null || input.deploymentUrl.length === 0) {
            return yield* Effect.fail(
              new IMessageError({
                reason: "validation",
                detail: "pairing needs a valid handle and deployment URL",
              }),
            );
          }
          const deploymentUrl = yield* validatePairingUrl(input.deploymentUrl);
      ...
      verifyPair: (input) =>
        Effect.gen(function* () {
          ...
          if (
            ...
            // Its group bindings die with it (lookupSpace would already skip
            // them via the registry join; this is hygiene).
            if (retired.length > 0) {
              yield* database.query(`DELETE FROM imessage_space_registry WHERE handle = $1`, [
                request.supersedes_handle,
              ]);
              yield* database.query(`DELETE FROM imessage_chat_registry WHERE handle = $1`, [
                request.supersedes_handle,
              ]);
            }
          }
      ...
      resolveActor: (secret) =>
        Effect.gen(function* () {
          ...
          const rows = yield* database.query(
            `SELECT handle, deployment_url, secret, verified_at::text AS verified_at
               FROM imessage_registry WHERE secret = $1 LIMIT 1`,
            [secret],
          );
          const decoded = yield* decodeRegistryRows(rows);
          const row = decoded[0];
          return row === undefined
            ? null
            : { handle: row.handle, deploymentUrl: row.deployment_url };
        }),
      ...
      authorizedSend: (input) =>
        Effect.gen(function* () {
          ...
          if (input.space === undefined) {
            return yield* sendText({ handle: row.handle }, text, input.phone);
          }
          // Space sends stay as narrow as handle sends: a deployment may only
          // post into group spaces its own paired owner has spoken in.
          const bound = yield* database.query(
            `SELECT space_id FROM imessage_space_registry WHERE space_id = $1 AND handle = $2`,
            [input.space, row.handle],
          );
          if (bound.length === 0) {
            return yield* Effect.fail(
              new IMessageError({
                reason: "router",
                detail: "that group space is not bound to this deployment's handle",
                status: 403,
              }),
            );
          }
          yield* sendText({ space: input.space }, text, input.phone);
        }),
      ...
      signedAttachmentFetch: (input) =>
        Effect.gen(function* () {
          ...
          if (
            handle === null ||
            input.access.id.trim().length === 0 ||
            input.access.conversation.trim().length === 0
          ) {
            return yield* Effect.fail(
          ...
          const file = yield* fetchInboundAttachmentBytes(input.providerAttachmentId, {
      ...
      signedOpaqueAttachmentFetch: (input) =>
        Effect.gen(function* () {
          yield* requireRouter;
          yield* ensureTables;
          const rows = yield* database.query(
            `SELECT r.secret
               FROM imessage_registry r
              WHERE $1 = 'dm:' || $2 || ':' || r.handle
              UNION ALL
             SELECT r.secret
               FROM imessage_space_registry s
               JOIN imessage_registry r ON r.handle = s.handle
              WHERE $1 = 'space:' || $2 || ':' || s.space_id
              LIMIT 1`,
            [input.conversationKey, input.phone],
          );
          const secret =
            typeof rows[0]?.secret === "string" ? rows[0].secret : null;
          const verification =
            secret === null
              ? { ok: false as const, reason: "unknown attachment owner" }
              : verifyOpaqueAttachmentAccess({
                  secret,
          ...
          const file = yield* fetchInboundAttachmentBytes(
            input.providerAttachmentId,
            {
              ...(input.phone.length > 0 ? { phone: input.phone } : {}),
              convert: input.access.convert,
            },
          );
          if (file === null) {
      ...
      shareContactAfterFirstInbound: (input) =>
        Effect.gen(function* () {
          yield* requireRouter;
          yield* ensureTables;
          const handle = normalizeHandle(input.handle);
          if (handle === null) return;
          const claimed = yield* database.query(
            `UPDATE imessage_registry
                SET contact_share_claimed_at = now()
              WHERE handle = $1
                AND contact_shared_at IS NULL
                AND (
                  contact_share_claimed_at IS NULL
                  OR contact_share_claimed_at < now() - interval '5 minutes'
                )
              RETURNING handle`,
            [handle],
          );
          if (claimed.length === 0) return;
          const sent = yield* shareContactCardContent(handle, input.phone).pipe(
            Effect.as(true),
          ...
          if (!sent) {
            yield* database.query(
              `UPDATE imessage_registry
                  SET contact_share_claimed_at = NULL
                WHERE handle = $1 AND contact_shared_at IS NULL`,
              [handle],
            );
            return;
          }
          yield* database.query(
            `UPDATE imessage_registry
                SET contact_shared_at = now(), contact_share_claimed_at = NULL
              WHERE handle = $1`,
            [handle],
          );
        }),
...
export const claimIMessageInbound = (input: {
  ...
  Effect.gen(function* () {
    return yield* (yield* IMessagePairing).claimInbound({
      ...input,
      attachments: input.attachments ?? [],
      chatType: input.chatType ?? "dm",
      role: input.role ?? (input.handle === input.ownerHandle ? "owner" : "guest"),
    });
  });
...
export const reconcileIMessageInbound = (input: {
  readonly messageId: string;
  readonly kind: "edited" | "unsent";
  readonly text?: string;
}): Effect.Effect<
  "pending_updated" | "already_started" | "missing",
  IMessageStoreError,
  IMessagePairing
> =>
  Effect.gen(function* () {
    return yield* (yield* IMessagePairing).reconcileInbound(input);
  });

export const discardIMessageInboundForReset = (input: {
  readonly ownerHandle: string;
  readonly spaceId: string;
  readonly resetMessageId: string;
  readonly resetAtMs: number;
  ...
  Effect.gen(function* () {
    return yield* (yield* IMessagePairing).discardInboundForReset(input);
  });

export const isIMessageInboundBatchCurrent = (
  messageIds: readonly string[],
): Effect.Effect<boolean, IMessageStoreError, IMessagePairing> =>
  Effect.gen(function* () {
    return yield* (yield* IMessagePairing).isInboundBatchCurrent(messageIds);
  });

export const tryAcquireIMessageDeliveryLock = (input: {
  readonly ownerHandle: string;
  readonly spaceId: string;
}): Effect.Effect<string | null, IMessageStoreError, IMessagePairing> =>
  Effect.gen(function* () {
    return yield* (yield* IMessagePairing).tryAcquireDeliveryLock(input);
  });

export const releaseIMessageDeliveryLock = (input: {
  readonly ownerHandle: string;
  readonly spaceId: string;
  readonly token: string;
}): Effect.Effect<void, IMessageStoreError, IMessagePairing> =>
  Effect.gen(function* () {
    return yield* (yield* IMessagePairing).releaseDeliveryLock(input);
  });
...
export const listIMessageTranscript = (
  limit = 100,
): Effect.Effect<readonly IMessageTranscriptEntry[], IMessageStoreError, IMessagePairing> =>
  Effect.gen(function* () {
    return yield* (yield* IMessagePairing).listTranscript(limit);
  });

export const activateIMessageSpace = (input: {
  readonly spaceId: string;
  readonly handle: string;
}): Effect.Effect<void, IMessageStoreError, IMessagePairing> =>
  Effect.gen(function* () {
    return yield* (yield* IMessagePairing).activateSpace(input);
  });

export const isIMessageSpaceActive = (input: {
  readonly spaceId: string;
  readonly handle: string;
}): Effect.Effect<boolean, IMessageStoreError, IMessagePairing> =>
  Effect.gen(function* () {
    return yield* (yield* IMessagePairing).isSpaceActive(input);
  });
...
export const sendIMessageCommand = (input: {
  readonly commandId: string;
  readonly phone: string;
  readonly target: IMessageTarget;
  readonly operation: IMessageOperation;
  readonly payload: unknown;
}): Effect.Effect<IMessageCommandResult, IMessageStoreError, IMessagePairing> =>
  Effect.gen(function* () {
    return yield* (yield* IMessagePairing).sendCommand(input);
  });

/**
 * Texts the owner — the paired handle — over iMessage. Pairing is optional,
 * so an unpaired deployment is not an error here: the send is skipped and
 * `false` comes back, letting proactive callers (fired reminders) treat
 * iMessage as a best-effort notification mirror.
 */
export const notifyOwnerOverIMessage = (
  text: string,
): Effect.Effect<boolean, IMessageStoreError, IMessagePairing> =>
  Effect.gen(function* () {
    const service = yield* IMessagePairing;
    const pairing = yield* service.verified();
    if (pairing === null) return false;
    yield* service.sendReply({ handle: pairing.handle, text });
    return true;
  });
...
export const shareIMessageContactAfterFirstInbound = (input: {
  readonly handle: string;
  readonly phone?: string;
}): Effect.Effect<void, IMessageStoreError, IMessageRouter> =>
  Effect.gen(function* () {
    return yield* (yield* IMessageRouter).shareContactAfterFirstInbound(input);
  });

export const bindIMessageSpace = (input: {
  readonly spaceId: string;
  readonly handle: string;
}): Effect.Effect<void, IMessageStoreError, IMessageRouter> =>
  Effect.gen(function* () {
    return yield* (yield* IMessageRouter).bindSpace(input);
  });

export const lookupIMessageSpace = (
  spaceId: string,
): Effect.Effect<
  { handle: string; deploymentUrl: string; secret: string } | null,
  IMessageStoreError,
  IMessageRouter
> =>
  Effect.gen(function* () {
    return yield* (yield* IMessageRouter).lookupSpace(spaceId);
  });

export const bindIMessageChat = (input: {
  readonly phone: string;
  readonly chatId: string;
  readonly handle: string;
}): Effect.Effect<void, IMessageStoreError, IMessageRouter> =>
  Effect.gen(function* () {
    return yield* (yield* IMessageRouter).bindChat(input);
  });

export const lookupIMessageChat = (input: {
  readonly phone: string;
  readonly chatId: string;
}): Effect.Effect<
  { handle: string; deploymentUrl: string; secret: string } | null,
...
> =>
  Effect.gen(function* () {
    return yield* (yield* IMessageRouter).lookupChat(input);
  });
...
export const resolveIMessageDeploymentActor = (
  secret: string,
): Effect.Effect<
  { handle: string; deploymentUrl: string } | null,
  IMessageStoreError,
  IMessageRouter
> =>
  Effect.gen(function* () {
    return yield* (yield* IMessageRouter).resolveActor(secret);
  });
...
export const fetchIMessageAttachmentSigned = (input: {
  ...
  IMessageStoreError | import("./imessage/store").RichStoreError,
  IMessageRouter | IMessageRichStoreService
> =>
  Effect.gen(function* () {
    const providerAttachmentId = yield* (yield* IMessageRichStoreService).resolveMessageRef({
      messageRef: input.access.id,
      phone: input.access.phone,
      conversationKey: input.access.conversation,
    });
    return yield* (yield* IMessageRouter).signedAttachmentFetch({
      ...input,
      providerAttachmentId,
    });
  });

export const fetchIMessageAttachmentOpaqueSigned = (input: {
  readonly access: OpaqueAttachmentAccess;
  readonly signature: string;
}): Effect.Effect<
  InboundAttachmentBytes,
...
> =>
  Effect.gen(function* () {
    const binding = yield* (yield* IMessageRichStoreService).inspectMessageRef(
      input.access.ref,
    );
    return yield* (yield* IMessageRouter).signedOpaqueAttachmentFetch({
      ...input,
      providerAttachmentId: binding.providerMessageId,
      phone: binding.phone,
