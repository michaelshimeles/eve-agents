...
/** Comfortably under OpenAI's 25 MB transcription cap; iMessage voice memos
 * rarely get anywhere close. */
const MAX_TRANSCRIBE_AUDIO_BYTES = 20 * 1024 * 1024;

/**
 * Transcribes one audio attachment (an iMessage voice memo, today) with
 * OpenAI's transcription endpoint, so a spoken message becomes the same
 * plain text the model already reads for a typed one. Reuses
 * gpt-4o-mini-transcribe, the model already trusted for the voice orb's live
 * transcription (see the `session.audio.input.transcription` field above).
 */
export function transcribeAudioAttachment(
  bytes: Uint8Array,
  mimeType: string,
  filename: string,
): Effect.Effect<AudioTranscript, VoiceError> {
  return Effect.gen(function* () {
    const apiKey = process.env.OPENAI_API_KEY ?? "";
    if (apiKey.length === 0) {
      return yield* Effect.fail(
    ...
    if (bytes.byteLength > MAX_TRANSCRIBE_AUDIO_BYTES) {
      return yield* Effect.fail(
        new VoiceError({
          reason: "openai",
          message: `Audio attachment is too large to transcribe (${bytes.byteLength} bytes)`,
        }),
      );
    }
    ...
    const response = yield* Effect.tryPromise({
      ...
      Effect.mapError((error) =>
        error instanceof VoiceError
          ? error
          : new VoiceError({ reason: "timeout", message: "OpenAI transcription request timed out" }),
      ),
    );
    if (!response.ok) {
      const detail = yield* Effect.tryPromise({
    ...
    if (typeof payload.text !== "string" || payload.text.trim().length === 0) {
      return yield* Effect.fail(
        new VoiceError({ reason: "openai", message: "Unexpected transcription response shape" }),
      );
    }
