...
function connectionConfig(relayToken?: string): {
  url: string;
  headers: Record<string, string>;
} {
  const direct = localComputerUsesDirectTunnel();
  const token = direct ? env("RUTH_LOCAL_MCP_TOKEN") : relayToken;
  if (token === undefined || token.length < 32) {
    throw new LocalComputerError({
      reason: "not_configured",
      detail: "connect the Mac under Manage -> Computer.",
    });
  }

  const accessId = direct ? env("RUTH_LOCAL_CF_ACCESS_CLIENT_ID") : undefined;
  const accessSecret = direct
    ? env("RUTH_LOCAL_CF_ACCESS_CLIENT_SECRET")
    : undefined;
...
export const LocalComputerLive = Layer.effect(
  LocalComputer,
  Effect.gen(function* () {
    ...
    return {
      task: (input, signal) =>
        Effect.gen(function* () {
          const relayToken = directLocalComputerConfigured()
            ? undefined
            : (
                yield* relay.agentCredential().pipe(
                  Effect.mapError(
                    (cause) =>
                      new LocalComputerError({
                        ...
                        detail:
                          ...
                          typeof cause === "object" &&
                          "_tag" in cause &&
                          cause._tag === "LocalComputerRelayError"
                            ? describeLocalComputerRelayError(
                                cause as LocalComputerRelayError,
                              )
          ...
          const config = connectionConfig(relayToken);
          return yield* Effect.tryPromise({
            try: () => runTask(input, config, signal),
            catch: (cause) =>
