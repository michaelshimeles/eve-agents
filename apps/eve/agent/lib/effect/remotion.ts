...
export interface RenderedVideo {
  ...
  /** ISO timestamp when the URL stops working (private Blob stores hand out
   * expiring presigned links); null when the link is permanent. */
  ...
  const store = Effect.tryPromise({
    try: async (): Promise<{
      ...
      if (process.env.BLOB_READ_WRITE_TOKEN) {
        const { uploadForSharing } = await import("../blob-share");
