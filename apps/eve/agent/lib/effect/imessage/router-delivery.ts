...
function attachmentIds(content: unknown): readonly string[] {
  if (content === null || typeof content !== "object") return [];
  const arm = content as { type?: unknown; id?: unknown; items?: unknown };
  if (arm.type === "attachment") {
    return typeof arm.id === "string" && arm.id.length > 0 ? [arm.id] : [];
  }
...
function replaceAttachmentIds(
  ...
  if (arm.type === "attachment" && typeof arm.id === "string") {
