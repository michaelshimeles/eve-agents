...
import { ACTIVE_CONNECTION_ROW, AgentcardStore } from "./agentcard";
  ...
  return {
    ...
    activateConnection: (input) =>
      Effect.sync(() => {
        ...
        next.set(input.connectionName, input.connection);
        next.set(ACTIVE_CONNECTION_ROW, input.active);
        if (
          input.previousConnectionName !== null &&
          input.previousConnectionName !== input.connectionName
        ) {
          next.delete(input.previousConnectionName);
        }
        rows.clear();
        for (const [name, value] of next) rows.set(name, value);
      }),
