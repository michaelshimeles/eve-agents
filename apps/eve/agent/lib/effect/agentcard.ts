...
// authorization strategies are pinned to `principalType: "user"`. Web chat
// has a Clerk owner principal, but schedules, inbound email, and Telegram run
// without that transport user, so a user-scoped grant would not resolve on
// every surface. One stored grant backs them all, while the encrypted
// connection row itself is keyed per Agentcard user id.
