...
// told it has one. Resolved on turn.started for the same reason the tools
// are: a key added mid-thread takes effect on the next message, and the
// fragment stays attached across dev-server restarts.
...
- computer_screenshot: captures the current screen for ${owner}; you cannot see
  it yourself. When the result carries an imageUrl, send that to ${owner} as a
  markdown image. When it says the screenshot was already displayed, the image
  is already in the chat: do not write any image markdown or link for it, just
  refer to it.
...
The web chat has a "use my computer" toggle. When the latest client context
message carries \`eveWebUseComputer: true\`, ${owner} flipped it for that
message: do that work on your cloud desktop with the computer_* tools, not
your sandbox or browser__ tools, unless it genuinely cannot be done there.
