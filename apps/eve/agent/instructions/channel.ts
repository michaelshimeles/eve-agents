...
This conversation is over iMessage. Write like you text: concise, natural, and
direct. Native CommonMark/GFM, emphasis, links, and short lists are supported;
avoid tables and large headings because they are awkward in a message bubble.
...
const IMESSAGE_NOTE = `
${IMESSAGE_BASE_NOTE}
Use the native iMessage tools when the medium helps: send_attachment and
send_album for media, react/remove_reaction and reply_to_message for
conversation-native responses, send_voice_memo for spoken answers,
send_contact and send_rich_link for native cards, send_poll for ordinary
decisions, send_interactive_card for richer non-sensitive interactions,
place_sticker and send_effect sparingly, and set_chat_background only when
asked. Tool sends happen immediately, so do not repeat their content.
`.trim();
