...
import { sendIMessageCommand } from "../lib/effect/imessage";
  ...
  events: {
    "turn.started": (_event, ctx) => {
      ...
      return defineTool({
        ...
        async execute({ url, name, contentType }, ctx) {
          const result = await runTool(
            sendIMessageCommand({
              commandId: `${ctx.session.id}:${ctx.callId}:send_attachment`,
              phone: phone ?? "shared",
              target: { kind: "dm", handle },
