...
const httpsUrl = z
  .string()
  .regex(/^[hH][tT][tT][pP][sS]:\/\//, "Use a public HTTPS URL")
  .url();

export const iMessageBackgroundInput = z
  .strictObject({
    background: z
      .union([z.literal("clear"), httpsUrl])
      .describe('Public HTTPS background URL, or the literal "clear".'),
  })
  .describe('Set a public HTTPS background URL, or pass "clear" to remove it.');

export const iMessageBackgroundRuntimeInput = z.strictObject({
  background: z.union([z.literal("clear"), httpsUrl]),
});
    ...
    "turn.started": (_event, ctx) => {
      ...
      return {
        ...
        send_album: defineTool({
          ...
          inputSchema: z.object({
            urls: z.array(httpsUrl).min(2).max(10),
          }),
        ...
        send_rich_link: defineTool({
          ...
          inputSchema: z.object({ url: httpsUrl }),
        ...
        set_chat_background: defineTool({
          ...
          inputSchema: iMessageBackgroundInput,
          execute: (input, toolCtx) => {
            const parsed = iMessageBackgroundRuntimeInput.safeParse(input);
            if (!parsed.success) {
              throw new Error('Provide a public HTTPS background URL or "clear"');
            }
            if (parsed.data.background === "clear") {
              return command("remove_background", {}, toolCtx);
            }
            return command("set_background", { url: parsed.data.background }, toolCtx);
          },
        }),
