...
export const iMessageBackgroundInput = {
  type: "object",
  properties: {
...
export const iMessageBackgroundRuntimeInput = z.union([
  z.strictObject({ url: httpUrl }),
      ...
      return {
        ...
        set_chat_background: defineTool({
          ...
          execute: (input, toolCtx) => {
            ...
            if (!parsed.success) {
              throw new Error("Provide either a background URL or clear: true");
            }
            if ("clear" in parsed.data) {
              return command("remove_background", {}, toolCtx);
            }
            return command("set_background", { url: parsed.data.url }, toolCtx);
          },
        }),
