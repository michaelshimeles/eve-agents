...
export default defineInstrumentation({
  ...
  setup: ({ agentName }) => {
    ...
    // The Braintrust Marketplace integration injects a Braintrust API key,
    // but production traces must still flow through Vercel's collector so the
    // project-scoped Trace Drain can preserve the complete workflow trace.
    // Direct export remains useful locally and outside Vercel.
    const useDirectBraintrustExporter =
      braintrustKey !== "" && !USE_VERCEL_PRODUCTION_COLLECTOR;
    if (
      braintrustKey === "" &&
      otlpEndpoint === "" &&
