...
//
// It also speaks the company REST API (client_credentials platform tokens,
// the email-code connect flow; the code is always 111111 like the real
// sandbox), so company mode runs against it end to end:
//
...
//   AGENTCARD_OWNER_EMAIL=owner@example.com npm run dev
//
...
function issueCompanyConnection() {
  ...
  return {
    ...
    user: { id: "user_stub", email: account.email, phone: null },
  };
}
...
async function handle(request, response) {
  ...
  // --- Company REST API ----------------------------------------------------
  // The companies product: client_credentials platform tokens and the
  // email-code connect flow. The code is always 111111, like the real
  // sandbox. Connection tokens land in the shared accessTokens map, so the
  // MCP endpoint accepts them exactly like personal ones.

  if (path === "/api/v2/oauth/token" && method === "POST") {
    const form = new URLSearchParams(await readBody(request));
    if (form.get("grant_type") !== "client_credentials" || !form.get("client_id")) {
  ...
  if (path.startsWith("/api/v2/connect/")) {
    ...
    const header = request.headers.authorization ?? "";
    const match = /^Bearer (.+)$/.exec(header);
    const platform = match === null ? undefined : platformTokens.get(match[1]);
    if (platform === undefined || platform.expires_at <= Date.now()) {
    ...
    if (path === "/api/v2/connect/start") {
      ...
      connectAttempts.set(id, { email: body.email });
      console.log(`[agentcard-stub] connect code for ${body.email}: ${COMPANY_CODE}`);
      return json(response, 201, {
    ...
    if (path === "/api/v2/connect/verify") {
      ...
      if (attempt === undefined) {
        return json(response, 400, { error: "invalid_connect_attempt" });
      }
      if (body.code !== COMPANY_CODE) {
        return json(response, 401, { error: "invalid_code" });
      }
    ...
    if (path === "/api/v2/connect/refresh") {
      if (!companyRefreshTokens.has(body.refresh_token)) {
        return json(response, 401, { error: "invalid_refresh_token" });
      }
