...
describe("sessionHasEventAt", () => {
  ...
  it("resolves current authorization headers for the stream probe", async () => {
    const fetchMock = vi.fn(streamingFetch(['{"type":"turn.started"}\n']));
    const resolveHeaders = vi.fn(async () => ({ authorization: "Bearer refreshed" }));
    vi.stubGlobal("fetch", fetchMock);

    await expect(sessionHasEventAt("session", 3, resolveHeaders)).resolves.toBe(true);
    expect(resolveHeaders).toHaveBeenCalledOnce();
    expect(fetchMock).toHaveBeenCalledWith(
      "/eve/v1/session/session/stream?startIndex=3",
