...
export async function requireWorkspaceRequest(
  request: Request,
  mutation = false,
): Promise<WorkspaceAuthentication> {
  const authentication = await authenticateWorkspaceRequest(request);
 ...
 * The public boundary is deliberately two-part: Clerk proves the configured
 * owner, then the durable thread row proves this session is theirs.
 */
