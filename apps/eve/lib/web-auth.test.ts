...
describe("owner web access", () => {
  ...
  it("does not accept removed surface-specific admin-token headers", async () => {
    vi.stubEnv("CLERK_OWNER_USER_ID", "user_owner");
    ...
    const response = await requireWebAuth(
      new Request("https://ruth.example/api/phone", {
        headers: {
          "x-card-admin-token": "obsolete",
          "x-imessage-admin-token": "obsolete",
          "x-phone-admin-token": "obsolete",
          "x-workspace-admin-token": "obsolete",
        },
      }),
    );
