...
// verifying one rotates the payment grant every surface spends with. Clerk
// protects browser calls; this independent token remains as an operational API
// alternative. The rules live in `lib/admin-auth.ts`; the in-chat connect tools
// are gated separately (owner-only turns) and never pass through these routes.
