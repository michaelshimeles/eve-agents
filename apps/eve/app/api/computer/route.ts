...
/**
 * The connection the browser should use. Orgo's websockify proxy only admits
 * browser origins it knows (orgo.ai itself and http://localhost:3000) and
 * cuts everything else off after the upgrade with 4001 "Origin not allowed",
 * so only a localhost page gets Orgo's own URL. A deployed page is pointed at
 * the same-origin relay (./ws), which dials Orgo from the server where no
 * Origin header is sent. The password rides along either way: the relay only
 * carries bytes, and VNC authenticates in-band.
 */
function browserConnection(request: Request, connection: LiveConnection | null): LiveConnection | null {
  ...
  if (origin.hostname === "localhost") return connection;
  return {
...
async function currentState(
  ...
  return Response.json({
    ...
    connection: browserConnection(request, connection),
    ...extra,
  });
}
