...
describe("workspace route authorization", () => {
  ...
  it("rejects the Clerk owner when the session is not owner-bound", async () => {
  ...
  it("allows the Clerk owner to reach only an owner-bound session", async () => {
  ...
  it("does not let the removed admin-token header bypass Clerk", async () => {
    ...
    const response = await readOverview(
      new Request(
        "https://ruth.example/api/workspace?sessionId=victim-session",
        { headers: { "x-workspace-admin-token": "owner-secret" } },
      ),
    );
