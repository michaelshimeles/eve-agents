...
import { resolveIMessageRef } from "@/agent/lib/effect/imessage/store";
import { runApp } from "@/agent/lib/effect/runtime";
import { handleV2Command } from "./v2";
...
// a deployment can only ever reach its own paired owner, or post plain text
// into group spaces bound to its own handle ({ text, space }). Never anyone
// else on the shared line. The body picks the content kind:
//
//   { text, space? }                          plain text (space = group chat)
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (
    ...
    const response = await handleV2Command(body, secret);
  ...
  if (reaction !== null) {
    let targetMessageId = reaction.targetMessageId;
    if (targetMessageId.startsWith("imr_")) {
      try {
        targetMessageId = await runApp(
          resolveIMessageRef({
            messageRef: targetMessageId,
            phone: phone || "shared",
            conversationKey: `dm:${phone || "shared"}:${handle}`,
          }),
        );
      } catch {
        return Response.json(
          { error: "The message reference does not belong to this conversation." },
          { status: 403 },
        );
      }
    }
