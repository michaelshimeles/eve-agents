...
export async function POST(request: Request): Promise<Response> {
  ...
  // The public app's stable owner scope is sealed into Blob's SDK-signed token
  // payload so the completion webhook can register the same owner without
  // trusting client metadata.
  let ownerId: typeof WEB_OWNER_PRINCIPAL_ID | null = null;
  if (body.type === "blob.generate-client-token") {
    ownerId = webRequestPrincipal().principalId;
  }
  try {
    const result = await handleUpload({
      ...
      onBeforeGenerateToken: async (pathname, clientPayload) => {
        ...
        if (ownerId === null) {
          throw new Error("Owner scope is required.");
        }
