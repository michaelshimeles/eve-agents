...
vi.mock("@/lib/web-auth", () => ({
  authenticateWebRequest: mocks.authenticateWebRequest,
}));
...
describe("/api/files authorization", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.authenticateWebRequest.mockReturnValue(owner);
    mocks.listChatFiles.mockResolvedValue([file]);
    mocks.registerChatFile.mockResolvedValue(file);
  });

  it("rejects an unauthenticated list before touching storage", async () => {
    mocks.authenticateWebRequest.mockReturnValue(
    ...
    expect(mocks.listChatFiles).not.toHaveBeenCalled();
  });

  it("scopes file listing to the authenticated owner", async () => {
    const response = await GET(new Request("https://ruth.example/api/files"));
