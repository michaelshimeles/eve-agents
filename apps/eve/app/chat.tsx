...
/**
 * The composer's computer toggle rides in the URL (`?computer=1`) so a reload
 * — or the ChatThread remount on a thread switch — keeps it on until it is
 * clicked off.
 */
const COMPUTER_PARAM = "computer";

function computerFromLocation(): boolean {
  return new URLSearchParams(window.location.search).has(COMPUTER_PARAM);
}
...
function ChatApp({ initialView }: { initialView: MainView }) {
  ...
  return (
    ...
    >
      ...
      {desktopOpen && (
        <DesktopDrawer onWidthChange={changeDesktopWidth} onClose={() => showDesktop(false)} />
      )}
...
function ChatThread({
  ...
  // rides along while it is on. Mirrored in the URL (`?computer=1`) so a
  // reload or thread switch keeps it.
  const [useComputer, setUseComputer] = useState(computerFromLocation);
  useEffect(() => {
    const url = new URL(window.location.href);
    if (url.searchParams.has(COMPUTER_PARAM) === useComputer) return;
    if (useComputer) url.searchParams.set(COMPUTER_PARAM, "1");
    else url.searchParams.delete(COMPUTER_PARAM);
    window.history.replaceState(null, "", url.pathname + url.search);
  }, [useComputer]);
  // Back/forward land on entries whose query carries the toggle as it was
  // then; restore it like the app-level handler does for `?desktop=`.
  useEffect(() => {
    function onPopState() {
      setUseComputer(computerFromLocation());
    }
    window.addEventListener("popstate", onPopState);
    return () => window.removeEventListener("popstate", onPopState);
  }, []);
 ...
 * to watch the agent work while the conversation carries on next to it. On
 * large screens its start edge drags to resize (the width lives in the
 * `--desktop-width` variable set by ChatApp, which also pads the content).
 */
function DesktopDrawer({
  ...
  return (
    <>
      ...
      <aside
        aria-label={`${AGENT_NAME}'s desktop`}
        className="fixed inset-y-0 end-0 z-50 flex w-full max-w-xl flex-col gap-3 border-s border-kumo-hairline bg-kumo-elevated p-4 shadow-xl lg:w-[var(--desktop-width)] lg:max-w-none"
      >
        <div
          role="separator"
          aria-orientation="vertical"
          aria-label="Resize the desktop panel"
          tabIndex={0}
          ...
          onPointerDown={(event) => {
            // Capturing keeps the drag alive over the VNC canvas and past the
